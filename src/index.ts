...
// KV storage map:
// prefs:global -> user theme, density, default landing preferences.
// health:snapshot -> latest aggregated site health generated by cron/manual checks.
// shortcuts:global -> quick terminal snippets shown on the dashboard.
// cache:site:{siteId} -> optional high-read site detail cache.
// cf:status -> short-lived Cloudflare status widget cache.
...
async function assertAccess(request: Request, env: Env): Promise<void> {
  ...
  if (!email) throw new HttpError(401, "Cloudflare Access identity required");
  const allowed = (env.ADMIN_EMAILS || "").split(",").map((item) => item.trim()).filter(Boolean);
  if (allowed.length && !allowed.includes(email)) throw new HttpError(403, "Email is not allowed");
}
